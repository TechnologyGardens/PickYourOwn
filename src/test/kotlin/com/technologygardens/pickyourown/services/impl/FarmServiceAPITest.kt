...
class FarmServiceAPITest {
    ...
    fun getFarmById() {
        ...
        Mockito.`when`(farmRepository.findById(ArgumentMatchers.anyString())).thenReturn(farmOpt)
        ...
        verify(farmRepository, Mockito.times(1)).findById(ArgumentMatchers.anyString())
    }

    @Test(expected = NotFoundException::class)
    fun getFarmById_NotFound() {
        Mockito.`when`(farmService.getFarmById(ArgumentMatchers.anyString())).thenReturn(null)
    ...
    fun deleteById() {
        ...
        verify(farmRepository, Mockito.times(1)).deleteById(ArgumentMatchers.anyString())
    }

    @Test(expected = NotFoundException::class)
    fun deleteById_NotFound() {
        Mockito.`when`(farmService.deleteById(ArgumentMatchers.anyString())).thenThrow(NotFoundException::class.java)
